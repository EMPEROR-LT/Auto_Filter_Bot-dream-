...
# ============================
ADMINS = [int(admin) if id_pattern.search(admin) else admin for admin in environ.get('ADMINS', '7597291420').split()] # Replace with the actual admin ID(s) to add
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '-1003615363281').split()]  # Channel id for auto indexing (make sure bot is admin)

LOG_CHANNEL = int(environ.get('LOG_CHANNEL', '-1002905473240'))  # Log channel id (make sure bot is admin)
BIN_CHANNEL = int(environ.get('BIN_CHANNEL', '-1002528459859'))  # Bin channel id (make sure bot is admin)
PREMIUM_LOGS = int(environ.get('PREMIUM_LOGS', '-1003174376872'))  # Premium logs channel id
DELETE_CHANNELS = [int(dch) if id_pattern.search(dch) else dch for dch in environ.get('DELETE_CHANNELS', '-1003722185571').split()] #(make sure bot is admin)
support_chat_id = environ.get('SUPPORT_CHAT_ID', '-1002803456095')  # Support group id (make sure bot is admin)
reqst_channel = environ.get('REQST_CHANNEL_ID', '-1002571645255')  # Request channel id (make sure bot is admin)
SUPPORT_CHAT = environ.get('SUPPORT_CHAT', 'https://t.me/Movie_Lord_Support')  # Support group link (make sure bot is admin)
...
# ============================
QR_CODE = environ.get('QR_CODE', 'https://vault.pictures/p/c87e5d4b4c614240988f527f5fa9abf1')    # QR code image for payments
OWNER_UPI_ID = environ.get('OWNER_UPI_ID', 'lokesh.your.papa@fam')    # Owner UPI ID for payments
...
SHORTENER_API = environ.get("SHORTENER_API", "a032057e9b47f40a3c7d32a8d25e8bc4367bce7a") # Shortener API key
SHORTENER_WEBSITE = environ.get("SHORTENER_WEBSITE", "gplinks.com") # Shortener website

SHORTENER_API2 = environ.get("SHORTENER_API2", "a032057e9b47f40a3c7d32a8d25e8bc4367bce7a")  # Shortener API key for second website
...
# ============================
GRP_LNK = environ.get('GRP_LNK', 'https://t.me/SUPREME_BOTz') # Group link for the bot
OWNER_LNK = environ.get('OWNER_LNK', 'https://t.me/SUPREME_BOTz') # Owner link for the bot
...
SESSION_NAME = str(environ.get('SESSION_NAME', 'SUPREME_BOTz'))
MULTI_CLIENT = False
name = str(environ.get('name', '꧁𓆩𝕊𝕌ℙℝ𝔼𝕄𝔼 𝔹𝕆𝕋z𓆪꧂'))
...
Bot_cmds = {
    ...
    "grp_broadcast": "ʙʀᴏᴀᴅᴄᴀsᴛ ᴀ ᴍᴇssᴀɢᴇ ᴛᴏ ᴀʟʟ ᴄᴏɴɴᴇᴄᴛᴇᴅ ɢʀᴏᴜps",

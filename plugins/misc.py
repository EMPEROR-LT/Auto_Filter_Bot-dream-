...
import logging
from database.ia_filterdb import supreme_botz_get_movies, supreme_botz_get_series
...
async def supreme_botz_list_movies(client, message):
    try:
        movies = await supreme_botz_get_movies()
        if not movies:
        ...
        logger.error(f"Error in supreme_botz_list_movies: {e}")
...
async def supreme_botz_list_series(client, message):
    try:
        series_data = await supreme_botz_get_series()
        if not series_data:
        ...
        logger.error(f"Error in supreme_botz_list_series: {e}")
        await message.reply("An Error Occurred ☹️", parse_mode=ParseMode.HTML)